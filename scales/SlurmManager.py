...
def check_run_finished(run_folder: Path) -> bool:
    ...
    if run_folder.exists() and result_file.exists() and info_file.exists():
        ...
        if int(res["train_steps"]) >= int(_info["train_target"]):
            return True
    return False
...
class SlurmManager:
    ...
    def submit(self):
        submit = subprocess.run(f"sbatch {self.script_path}", shell=True, capture_output=True, text=True, input="y")
        self.job_id = submit.stdout.split(" ")[-1].strip()